...
module.exports = function (req, res, urlPieces, model, config) {
	...
	if (urlPieces.length < 2) {
		...
		var result = {};
		var _model = model.authorizedWhere ? model.authorizedWhere(req) : model;
		...
		if (hasTimestamps.indexOf(config.deletedAttribute) >= 0 && (!req.hardDelete && !config.hardDelete || req.hardDelete === false)) {
			...
			promise = _model.save(updatedData, { method: 'update' }).then(function (savedModel) {
				res.json(result);
			});
		} else {
			promise = _model.destroy({ require: true }).then(function (destroyedModel) {
				res.json(result);
			});
		}
